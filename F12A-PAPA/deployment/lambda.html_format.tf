...
resource "null_resource" "build_html_format" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/html_format/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "html_format" {
  type        = "zip"
  output_path = "${path.module}/../code/html_format.zip" # TODO: change here
  source_dir  = "${path.module}/../code/html_format"     # TODO: change here

  depends_on = [
    null_resource.build_html_format # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.html_format.
...
resource "aws_lambda_function" "html_format" {
  filename      = data.archive_file.html_format.output_path
  function_name = "${var.group_name}_${terraform.workspace}_html_format" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.html_format.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "html_format" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.html_format.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/html_format" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "html_format" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.html_format.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "html_format" {
  api_id    = var.gateway_api_id
  route_key = "POST /${var.group_name}/html_format" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.html_format.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "html_format_log" {
  name              = "/aws/lambda/${aws_lambda_function.html_format.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
